...
class PointCloudDataset(Dataset):
    ...
    def augmentation_transform(self, points, normals=None, verbose=False):
        ...
        if points.shape[1] == 3:
            if config.SETTING['augment_rotation'] == 'vertical':
            ...
            elif config.SETTING['augment_rotation'] == 'all':
        ...
        min_s = config.SETTING['augment_scale_min']
        max_s = config.SETTING['augment_scale_max']
        if config.SETTING['augment_scale_anisotropic']:
            scale = np.random.rand(points.shape[1]) * (max_s - min_s) + min_s
        else:
            scale = np.random.rand() * (max_s - min_s) - min_s

        # Add random symmetries to the scale factor
        symmetries = np.array(config.SETTING['augment_symmetries']).astype(np.int32)
        symmetries *= np.random.randint(2, size=points.shape[1])
        scale = (scale * (1 - symmetries * 2)).astype(np.float32)

        noise = (np.random.randn(points.shape[0], points.shape[1]) * config.SETTING['augment_noise']).astype(np.float32)
    ...
    def classification_inputs(self,
        ...
        r_normal = config.SETTING['first_subsampling_dl'] * config.SETTING['conv_radius']
        ...
        for block_i, block in enumerate(arch):
            ...
            if layer_blocks:
                ...
                if np.any(['deformable' in blck for blck in layer_blocks]):
                    r = r_normal * config.SETTING['deform_radius'] / config.SETTING['conv_radius']
                    deform_layer = True
                else:
                    r = r_normal
            ...
            if 'pool' in block or 'strided' in block:
                ...
                dl = 2 * r_normal / config.SETTING['conv_radius']
