...
def block_decider(block_name,
    ...
    if block_name == 'unary':
        return UnaryBlock(in_dim, out_dim, config.SETTING['use_batch_norm'], config.SETTING['batch_norm_momentum'])
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind):
        ...
        current_extent = radius * config.SETTING['KP_extent'] / config.SETTING['conv_radius']
        ...
        self.KPConv = KPConv(config.SETTING['num_kernel_points'],
                             config.SETTING['in_points_dim'],
                             in_dim,
                             out_dim // 2,
                             current_extent,
                             radius,
